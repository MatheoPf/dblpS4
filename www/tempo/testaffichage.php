...
if (isset($_GET['nom'])) {
 ...
 $req_get_auteur = "Select pid from analysegeo._auteurs where nom = '.$nom_auteur.'"; //on recupere le pid de l'auteur
 $pid ;
$req_get_publi_avec_auteur = "SELECT * from analysegeo.a_ecrit where pid =" .$pid. ";"; //on recupere tous les publications (id_dblp) que l'auteur a ecrit
$id_dblp;
$req_revues_avec_pid = "SELECT * FROM analysegeo._revues where id_dblp = id_dblp"; //on recupere les infos
 $req_conferences_avec_pid = "SELECT * FROM analysegeo._conferences where id_dblp = id_dblp";
 ...
 try {
     ...
     $stmt_get_auteur = $conn->prepare($req_get_auteur);
     $stmt_revues->execute();
     $pid_lauteur = $stmt_get_auteur->fetchAll(PDO::FETCH_ASSOC);

     $stmt_get_publi_avec_auteur = $conn->prepare($req_get_publi_avec_auteur);
     $stmt_get_publi_avec_auteur->execute();
     $id_dblp = $stmt_revues->fetchAll(PDO::FETCH_ASSOC);


    foreach ($id_dblp as $id) {
        $stmt_revues = $conn->prepare($req_revues_avec_pid);
        $stmt_revues->execute();
        $revues = array_merge($revues, $stmt_revues->fetchAll(PDO::FETCH_ASSOC));
    }

    foreach ($id_dblp as $id) {
        $stmt_conference = $conn->prepare($req_conferences_avec_pid);
        $stmt_conference->execute();
        $revues = array_merge($conference, $stmt_conference->fetchAll(PDO::FETCH_ASSOC));
    }
